...
from array_read_write import read_numpy_array, read_laser_calibration, read_camera_calibration
import os
...
def get_line(one, two, image_size):
    ...
    x2 = image_size[1]-1
    y2 = int(slope * (x2-one[0]) + one[1])

    if (y2 > image_size[0]-1):
        y2 = image_size[0]-1
        x2 = int((y2-one[1])/slope + one[0])
    return (x1,y1), (x2,y2)

def return_line(laser_path: Path, calibration_path: Path, image_size: tuple):

    laser_position, laser_orientation = read_laser_calibration(laser_path.as_posix())
    calibration_matrix, distortion_coeffs = read_camera_calibration(calibration_path.as_posix())

    first_point = laser_position + 1 * laser_orientation
    second_point = laser_position + 2 * laser_orientation
    ...
    return get_line(first_2d_tup, second_2d_tup, image_size)
...
def view_line(laser_path: Path, calibration_path: Path, data_path: Path):
    ...
    for file in os.listdir(data_path.as_posix()):
        ...
        image_size = tuple(list(np.shape(img))[0:2])
        start_point, end_point = return_line(laser_path, calibration_path, image_size)

        cv2.namedWindow("Resized_Window", cv2.WINDOW_NORMAL)

        cv2.line(img, start_point, end_point, color=(0,0,255), thickness=1) 
