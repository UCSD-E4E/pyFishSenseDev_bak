...
def display_detection(laser_path: Path, 
        ...
        contours = detect_laser_raw(processed_image, vanishing_point)   

        for cnt in contours:
            cv2.drawContours(processed_image,[cnt],0,(0,0,255),thickness=10)
...
def display_filtered_image(laser_path: Path, 
    ...
    processor = imageProcessing()
    processed_image, _ = processor.applyToImage(filepath, params)
    _, filtered_image = edgeDetection(processed_image, 500, 0, None, channel='r')
...
data_path = Path("C:/Users/Hamish/Documents/E4E/Fishsense/fishsense-lite-python-pipeline/data/4_12_canyonview/fred_green")
...
for file in os.listdir(data_path.as_posix()):
    ...
    # if i > 2:
    #     #display_filtered_masked_image(laser_path, calibration_path, filepath, vanishing_point, params_path)
    #     #display_detection(laser_path, calibration_path, filepath, vanishing_point, params_path)
    #     display_masked_image(laser_path, calibration_path, filepath, vanishing_point, True, params_path)
    # i += 1