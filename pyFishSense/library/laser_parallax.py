...
def image_coordinate_to_projected_point(
        ...
        focal_length_mm: float) -> np.ndarray:
...
def compute_world_points(
        ...
        laser_axis: np.ndarray,
        camera_params: tuple,
        image_coordinates: np.ndarray) -> np.ndarray:   
    projected_points = image_coordinate_to_projected_point_vec(
        image_points=image_coordinates,
        pixel_pitch_mm=camera_params[3],
        focal_length_mm=camera_params[0]
    )
    final_laser_axes = -1 * projected_points / np.linalg.norm(projected_points,axis=1)[...,np.newaxis]
    point_constants = ((final_laser_axes @ laser_origin) - ((laser_axis.T @ laser_origin) * (laser_axis @ final_laser_axes.T))) / \
